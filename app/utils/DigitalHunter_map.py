...
def plot_map_with_geometry(coords: list, buffer,
                           shapefile_path="utils/ne_50m_admin_0_countries.shp"):
        
    countries = gpd.read_file(shapefile_path)

    fig, ax = plt.subplots(figsize=(30, 30))

    # coords = [(35.0, 32.0), (35.2, 32.5), (35.5, 33.0)]
    ...
    if len(coords) == 1:
        x, y = coords[0]
        ax.plot(x, y, marker='o', color='blue', markersize=2, label='Point')

    elif len(coords) == 2:
        x, y = coords[0], coords[1]
        ax.plot(x, y, marker='o', color='blue', markersize=2, label='Point')
        ax.scatter(x[0], y[0], color='green', s=30)  # הצגת הנקודות על הקו
        ax.scatter(x[-1], y[-1], color='red', s=30)  # הצגת הנקודות על הקו
